...
function modulesPath(context: vscode.ExtensionContext): string {
  ...
  export function generateCssFile(context: vscode.ExtensionContext, str: string): void {
    console.log(str)
    ...
    for (const fileType in byFileType) {
      ...
      style += `.tab[title$=".${formatTabTitle(fileType)}" i]{background-color:${byFileType[fileType].backgroundColor} !important; opacity:${byFileType[fileType].opacity || "0.6"};}
          .tab[title$="${formatTabTitle(fileType)}" i] a,.tab[title$="${formatTabTitle(fileType)}" i] .monaco-icon-label:after,.tab[title$="${formatTabTitle(fileType)}" i] .monaco-icon-label:before{color:${byFileType[fileType].fontColor} !important;}`;
    }
  
    for (const directory in byDirectory) {
      if (directory == "my/directory/") continue;
      const formattedTitle = directory.replace(/\\/g, "\\\\");
      style += `.tab[title*="${formatTabTitle(formattedTitle)}" i]{background-color:${byDirectory[directory].backgroundColor} !important; opacity: ${byDirectory[directory].opacity || "0.6"};}
          .tab[title*="${formatTabTitle(formattedTitle)}" i] a,.tab[title*="${formatTabTitle(formattedTitle)}" i] .monaco-icon-label:after,.tab[title*="${formatTabTitle(formattedTitle)}" i] .monaco-icon-label:before{color:${byDirectory[directory].fontColor} !important;}`;
    }
    ...
    for (const i in tabs) {
      if (colorsData[i]) {
        ...
        activeSelectorsArr.push(..._colorTabs.map(function (a: string) {
          console.log("Jakobi")
          console.log(a)
...
export function reloadCss(): void {
  ...
  export function setColor(context: vscode.ExtensionContext, color: string, title: string): void {
    const storage = new Storage(context);
    if (storage.get("patchedBefore")) {
      if (storage.get("secondActivation")) {
        var colors = {[color]: {background: color, color: "white"}}
        storage.addCustomColor(colors);
        storage.addTabColor(color, title.replace(/\\/g, "\\\\"));
        generateCssFile(context, "Here1");
        reloadCss();
      } else {
...
export function unsetColor(context: vscode.ExtensionContext, title: string): void {
  const storage = new Storage(context);
  if (storage.get("patchedBefore")) {
    if (storage.get("secondActivation")) {
